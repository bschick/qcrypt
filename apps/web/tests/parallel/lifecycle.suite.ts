...
import { testWithAuth, toggleCredentials, deleteFirstPasskey, deleteLastPasskey, expectPrfBadge } from '.././common';
...
export function lifecycleSuite(prf: boolean): void {
   ...
   testWithAuth(`${label}: regenerate recovery words`, async ({ authFixture }) => {
      ...
      await expect(page.getByRole('button', { name: /I saved my recovery words securely/ })).toBeVisible({
         timeout: 10000,
      });
