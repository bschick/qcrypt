...
async function recomputeUserCred(
  ...
  const prfFirst = (assertion.clientExtensionResults as { prf?: { results?: { first?: string } } }).prf?.results?.first;
  if (!prfFirst) {
...
export const testWithAuth = test.extend<{authFixture: AuthFixture}>({
  authFixture: async ({ page }, use, testInfo) => {
    ...
    const addPasskey = async (
      ...
      active = authenticator.emulator;
      const verifyPromise = page.waitForResponse((r) =>
        (r.url().includes('/v1/passkeys/verify') || r.url().includes('/v1/reg/verify') || r.url().includes('/v1/recover/verify')) &&
