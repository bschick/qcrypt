...
describe('CLI App', () => {
    ...
    const execCli = (command: string, input?: string): SpawnSyncReturns<string> => {
        const cmd = `node ${cliPath} ${command}`;
    ...
    describe('input formats', () => {
        ...
        it('should roundtrip with binary stdin', () => {
            ...
            const binData = fs.readFileSync(binEnc);
            const result = spawnSync('sh', ['-c', `node ${cliPath} dec --cred ${userCred} --silent --outfile ${dec} --pwds pass`], {
                encoding: 'buffer',
                input: binData
            });
            expect(result.status).toBe(0);
        ...
        it('should pipe enc output to info', () => {
            const cmd = `echo "${clearText}" | node ${cliPath} enc --cred ${userCred} --silent --iters 1000000 --pwds pass | node ${cliPath} info --cred ${userCred} --silent`;
        ...
        it('should pipe enc output to dec', () => {
            const cmd = `echo "${clearText}" | node ${cliPath} enc --cred ${userCred} --silent --iters 1000000 --pwds pass | node ${cliPath} dec --cred ${userCred} --silent --pwds pass`;
            const result = spawnSync('sh', ['-c', cmd], { encoding: 'utf-8' });
            expect(result.status).toBe(0);
