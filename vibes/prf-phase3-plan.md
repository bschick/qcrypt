...
> **E2E update (2026-07-15):** the E2E pass is done and tracked separately in `vibes/prf-phase3-e2e-plan.md`. It replaced the CDP virtual authenticator with the nid-webauthn-emulator BrowserInjection (CDP can't persist PRF `credRandom` across a reload); all `parallel/` specs migrated + green (62), keeper accounts moved to the emulator FileRepository and verified end-to-end. **E2E phase now fully COMPLETE (2026-07-24):** keeper PRF-mode validation added (`keeperPrf` map + `sequential/keepers.spec` credentials-drawer badge check), prod KeeperOne mis-provision (created PRF by mistake, passed the pre-badge suite) found + re-provisioned no-PRF, `E2E_CREDS_B64` regenerated, full suite green on both hosts; both upstream nid PRs accepted unchanged (in the next nid release).

> **Docs update (2026-07-18) — committed:** Step 14 protocol doc (`main.tex`) content-complete (see Step 14). Flow SVGs (`assets/flow/v7`): PRF touches **only** `obtain_enc_input`/`obtain_dec_input` — step 3 relabeled to the mode-neutral **"obtain $u_c$"** (old `S\to u_c` wrongly implied the server ships plaintext); the other 28 diagrams are untouched because `u_c` is a linked input everywhere and the message-cipher algorithm is PRF-independent. Owner considered but did **not** decompose the `cd_p` unwrap into the diagram (it's the full `cd_x` block-decrypt under `p_R`, i.e. `MasterKeyKeyProvider`, which has no single LaTeX symbol — abstracted away instead). After any flow-SVG edit run `pnpm svgo:flow` (tags clickable regions + regenerates `flow.config.ts` integrity hashes; PDFs export at **255 PPI** per `assets-src/protocol_image_creation.txt`). **Step 5b PRF-case telemetry stays PUNTED.** Nothing here changes the production rollout: Phase 3 is code+docs complete and awaiting the owner-managed Deploy A → B → client sequence + real-device sign-off.

> **Docs update (2026-07-19) — committed:** `apps/server/API.md` brought fully current — Phase 3 (new `passkeyUserCredEnc`/`recoveryUserCredEnc`/`userCredPubKey` request fields split into `RegVerify`/`RecoverVerify`/`AddVerify` models; new `POST /v1/recover/verify`; `prf` on `UserInfo`, `RecoverInfo`, `passkeyUserCredEnc` on `LoginUserInfo`, `hasRecoveryId`→`hasRecoveryKey`; removed the dead `?usercred=`/`?recovery=` query params) **and** the stale Phase 2 bits (recover2 request corrected to `{userId, challenge, signature}`; added `POST /v1/recover2/challenge` and `PUT /v1/recover2/key`; all structure refs linked). The **Authorization section now documents the `x-proof` header** (`signature,timestamp,nonce`, ML-DSA sig over the request via a userCred-derived key), and every authorized endpoint lists it — with `GET /v1/session` = cookie + proof but **no** csrf (it issues the csrf). Consumer `overview.component.html` gained the "zero-knowledge / everything needed to unlock stays on your device" copy + a "Secure recovery from lost passkeys" bullet, hedged to new/supported-passkey accounts and to ship **with** the client, not ahead of prod. **Enforcement correction:** the userCred proof is **no longer observe-only in code** — there is no `PROOF_ENFORCE` flag and `verifyProof` is hard-coded to throw 401 on every `authorize:true` route. Per the owner it enforces the moment this build deploys to prod; prod still runs the earlier observe-only version until then.
...
**Next — the owner-managed Step 13 rollout; engineering + docs are DONE.** (Superseded plan note: this paragraph used to sketch the "next session" as a CDP-driven E2E pass — that pivoted to the nid-emulator BrowserInjection approach and is complete; see `vibes/prf-phase3-e2e-plan.md`. Step 14 protocol doc is done; Step 5b telemetry punted.) What's left: PR `prf-phase3`→`main` + green CI (force via `workflow_dispatch`), recreate prod keepers + regen `E2E_CREDS_B64`, then **Deploy A** (model+types+`getSessionKey` cutover → one-time forced re-login; verify clean re-auth) → **Deploy B** (server PRF handlers, inert until client) → **ship the client** (PRF create/login/recover/add + fallback UI) → **real-device Step 12 checklist** (Apple/GPM/YubiKey/Windows Hello). Non-blocking: 2 upstream nid PRs + JA docs.
...
### Step 14 — Protocol doc (`apps/web/src/assets-src/main.tex`) — DONE (2026-07-18)

Documents client-generated `userCred`, the per-passkey + recovery PRF ciphertexts, the `Users.prf` mode flag, the fixed global PRF salt + the `SHA-256("WebAuthn PRF"‖0x00‖salt)` mapping, that the server never holds plaintext `userCred` for PRF accounts, and the all-or-nothing invariant. Approach (owner-directed, exacting): the three at-rest `userCred` ciphertexts (`cd_u` session cache, `cd_p` per-passkey, `cd_r` recovery) are the **same construction keyed differently**, so the old `$u_c$ Encryption/Decryption by B` stanzas were **generalized to `… Under $k$`** with `$(k, cd_x) \in \{(k_L,cd_u),(p_R,cd_p),(u_r\|u_i,cd_r)\}$`. Ceremonies are written **PRF-primary + a `\textit{No-PRF fallback}` note** (not inline `if/else` forks — that read as pseudocode); added a new **Add Passkey** stanza and retitled **Authorization of B → of A** (A is passive there), with the state-changing nonce-uniqueness check moved to a trailing note (`b &= (n_P unknown)` + store). New symbols `p_R` (PRF output), `p_S` (salt), `cd_p`, `cd_r`, `cd_x`, generalized master `k`. Compiles clean. Authoring mechanics (255 PPI export, the keep-together `stanza` env, `pnpm svgo:flow` for the flow SVGs) are noted in `apps/web/src/assets-src/protocol_image_creation.txt` + `apps/web/scripts/`. **The owner renders + uploads the PDFs.**
